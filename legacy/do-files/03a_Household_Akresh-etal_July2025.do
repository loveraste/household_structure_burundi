...
* --- --- --- --- --- --- --- --- ---
		...
		* Column subtitles
		...
		putexcel C4 = "Observations", italic, 
		putexcel D4 = "Share of migration", 
		putexcel E4 = "Observations", italic, 
...
* --- --- --- --- --- --- --- --- ---
	...
	*** As ALL tables are at the household level ***
	keep if hh==1
	xtset id_hh year
		...
		* Write variable labels
		local row_lbl = 5
		foreach v of varlist $tables_hh {
		...
		* --- Headers ---
		putexcel B2:L2 = "Table 4: Baseline results. Armed Conflict and Non-Marital Migration, Household-level Analysis", merge hcenter border(bottom)
		...
		* --- Footnote ---
		putexcel B31:L31 = ///
		"Notes - This table presents our baseline regression for household migration. Robust standard error, clustered at Village level. * p<0.10  ** p<0.05  *** p<0.01. The dependent variable takes value one when at least one household member migrates due to no-marriage reason in a given year. Violence in a given year (yes=1) takes value one when the number of casualties in a given year is positive, 0 otherwise. Number of casualties includes individuals killed or wounded divided by 100. Index of Asset Loss refers to PCA of losses. Data Source: 2007 Burundi Priority Panel Survey.", ///
		merge txtwrap border(top)
...
* --- --- --- --- --- --- --- --- --- --- --- --- ---	
		...
		* --- Headers ---
		putexcel B2:L2 = "Table 7. Armed Conflict and Non-Marital Migration using lags, Household-level Analysis", merge hcenter border(bottom)
		putexcel B3 = "Dependent Variable: At least one household member migrates outside household in a given year (yes=1)", txtwrap hcenter vcenter
		putexcel B3:L3, border(bottom, double)
		...
		* --- Summary stats labels ---
		...
		putexcel B48 = "Household Fixed Effect", italic
		...
		* --- Footnote ---
		putexcel B51:L51 = ///
		"Notes - This table presents our baseline regression for household migration. Robust standard error, clustered at Village level. * p<0.10  ** p<0.05 *** p<0.01. The dependent variable, at least one houhold member migrates outside household in a given year (yes=1), takes value one when at least one household member migrate due to no-marriage reason in a giving year. Violence in a given year (presence=1)   takes value one when the number of casualties in a given year is positive, 0 otherwise. Number of casualties in a given year includes the number of individuals killed or wounded in a given year, divided by 100 people.Index of Asset Related Losses refers - PCA -  refers to the first component from a Principal Component Analysis for Theft of money (yes=1), Theft or destruction of goods (yes=1), and Destruction of house (yes=1)  for a household in a given year. Data Source: 2007 Burundi Priority Panel Survey.", ///
		merge txtwrap border(top)
...
* --- --- --- --- --- --- --- --- --- --- --- --- ---	
		...
		* Write variable labels
		local row_lbl = 5
		foreach v of varlist $base_vars5 {
			local lbl : variable label `v'
			putexcel B`row_lbl' = "`lbl'"
			local row_lbl = `row_lbl' + 2
		}
		local row_lbl = 10
		foreach v of varlist $pca_vars5 {
			local lbl : variable label `v'
			putexcel B`row_lbl' = "`lbl'"
			local row_lbl = `row_lbl' + 2
		}
		...
		* --- Headers ---
		putexcel B2:F2 = "Table 10: Baseline results. Armed Conflict and Non-Marital Migration, Household-level Analysis", merge hcenter border(bottom)
		putexcel B3 = "Dependent Variable: At least one household member migrates outside household in a given year (yes=1)", txtwrap hcenter vcenter
		...
		* --- Summary stats labels ---
		...
		putexcel B16 = "Household Fixed Effect", italic
...
* --- --- --- --- --- --- --- --- --- --- --- --- ---	
		...
		* Run regressions interacting each exposure and intensity with each PCA index
		...
		foreach x in $base_vars6 {
		    ...
		    qui xtreg d_leave_hh c.`x'##c.`z' i.year province_trend, fe cluster(reczd)
			eststo model_`count'
			qui summarize d_leave_hh if e(sample)
			qui estadd scalar depmean = r(mean)
			local ++count
		...
		* Write variable labels
		...
		local row_lbl = 12
		foreach v of varlist $pca_vars6 {
			local lbl : variable label `v'
			putexcel B`row_lbl' = "`lbl'"
			local row_lbl = `row_lbl' + 2
		}
		...
		* --- Headers ---
		putexcel B2:H2 = "Table 13. Baseline results. Armed Conflict and Non-Marital Migration, Household-level Analysis", merge hcenter border(bottom)
		putexcel B3 = "Dependent Variable: At least one household member migrates outside household in a given year (yes=1)", txtwrap hcenter vcenter
		...
		* --- Summary stats labels ---
		...
		putexcel B31 = "Household Fixed Effect", italic
...
* --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---	
		...
		* Run regressions and store
		...
		foreach x in $base_vars7 {
			foreach z in $pca_vars7 {
				qui xtreg d_leave_hh c.`x'##c.`z' c.lag_`x'##c.lag_`z' i.year province_trend, fe cluster(reczd)
				eststo model_`count'
				qui summarize d_leave_hh if e(sample)
				qui estadd scalar depmean = r(mean)
				local ++count
		...
		* --- Headers ---
		putexcel B2:H2 = "Table 16. Armed Conflict and Non-Marital Migration using lags, Household-level Analysis", merge hcenter border(bottom)
		putexcel B3 = "Dependent Variable: At least one household member migrates outside household in a given year (yes=1)", txtwrap hcenter vcenter
		putexcel B3:H3, border(bottom, double)
		...
		* --- Summary stats labels ---
		...
		putexcel B53 = "Household Fixed Effect", italic
