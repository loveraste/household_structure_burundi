...
*-------------------------------------------------------------------
* Models - SIGNIFICANT RESULTS ONLY (POSTFILE METHOD)
*------------------------------------------------------------------------
	keep if n_hh == 1
...
* --- Setup postfile for efficient result collection ---
postfile results str80 var_name double estimate double stderr double pvalue str3 significant str30 sample_type str30 interaction_var double n_obs double r2 using "$path_results/significant_results_postfile", replace
			...
			forvalues col = 1/`ncols' {
				...
				if `pval' < 0.10 {
					...
					* Post result directly
					post results ("`coef_name'") (`est') (`se') (`pval') ("`sig_level'") ("`sample_label'") ("`z'") (`n_obs') (`r2')
				}
			}
		}
	}
...
* --- Close postfile and convert to dataset ---
postclose results

* --- Load results and export to Excel ---
use "$path_results/significant_results_postfile", clear
...
if _N > 0 {
	...
	* Convert to Excel
	...
	di "  Filas: " _N
	di "  Columnas: variable, estimado, error estándar, p-value, significancia, tipo de muestra, variable de interacción, n_obs, r2"
}
else {
...
cap erase "$path_results/significant_results_postfile.dta"
